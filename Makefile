...
multi_build:
	...
	GOOS=darwin GOARCH=arm64 CGO_ENABLED=0 go build -o ./bin/darwin_arm64/vault-plugin-$(name)-$(plugin_type)_v$(version) cmd/vault-plugin-secrets-$(name)/main.go
	GOOS=darwin GOARCH=amd64 CGO_ENABLED=0 go build -o ./bin/darwin_amd64/vault-plugin-$(name)-$(plugin_type)_v$(version) cmd/vault-plugin-secrets-$(name)/main.go
	GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -o ./bin/windows_amd64/vault-plugin-$(name)-$(plugin_type)_v$(version).exe cmd/vault-plugin-secrets-$(name)/main.go
	GOOS=windows GOARCH=386 CGO_ENABLED=0 go build -o ./bin/windows_386/vault-plugin-$(name)-$(plugin_type)_v$(version).exe cmd/vault-plugin-secrets-$(name)/main.go
zip:
	pwd
	...
	zip -j ./bin/vault-plugin-$(name)-$(plugin_type)_v$(version)_linux_amd64.zip ./bin/linux_amd64/vault-plugin-$(name)-$(plugin_type)_v$(version)
